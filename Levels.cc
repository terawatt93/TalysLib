...
{  //функция должна инициализировать вычисления для каждого из требуемых beta и рисовать результаты вычисления упругого и первого неупругого рассеяния нейтронов (+сохранять в тектсовых файлах)
   ...
   {   cout<<"///////////// for i = "<<i<<" //////////////\n";
       ...
       ///string FilenameInel=getenv("TALYSDIR")+"/CalculationResults/def_graphs_inel/"+to_string(t.Get())+"/"+FileName+".txt";
       
       /*ifstream ifs(FilenameEl.c_str());
		if(ifs.good())//если файл с этим именем существует
		{
			TDatime t;
			string NewFileName=getenv("TALYSDIR")+"/CalculationResults/def_graphs_el/"+to_string(t.Get())+"/"+FileName+".txt";
			system(("mv "+Filename+" "+NewFileName).c_str());
			ifs.close();
		}*/
		
		///ofstream ofs(FilenameEl.c_str());//создать/открыть файл с именем(?)
