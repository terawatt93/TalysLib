...
\subsubsection{double Eval(ENDFTabledouble *x, double *p)}
...
\subsection{ENDF}
Работа с ENDF в TalysLib организована с помощью классов \textbf{ENDFFile, ENDFTable}, содержащих всю информацию из ENDF файла. \textbf{ENDFFile} хранит набор (map) таблиц \textbf{ENDFTable}, индексируемый по MF и MT как $i=MF*1000+MT$. Для поиска и получения указателя на нужную таблицу используется метод \textbf{ENDFFile::FindENDFTable(int MF,int MT)}, в случае отсутствия такой таблицы в наборе будет возвращен нулевой указатель. Аналогичная функция \textbf{ENDFFile::GetENDFTable(int MF,int MT)}, которая вернет указатель на нужную таблицу, создав ее при необходимости.
Открытие и считывание файла осуществляется с помощью метода \textbf{ENDFFile::Read(string filename)}, в котором также выполняется разбор содержимого на таблицы.

Каждая таблица представляет из себя комбинацию из заголовка (\textbf{Header}) и тела (\textbf{ENDFBasicTable::vector<vector<double> > Table}), для которой реалиованы методы произвольного доступа к ячейкам по двум индексам (\textbf{double\& ENDFBasicTable::at(unsigned int Column, unsigned int Row)}) и (\textbf{double* ENDFBasicTable::Get(unsigned int Column, unsigned int Row)}), также из нее могут быть получены векторы строк и столбцов с помощью функций \textbf{vector<double> ENDFBasicTable::GetColumn(unsigned int Column) и \textbf{vector<double> ENDFBasicTable::GetRaw(unsigned int Raw)}.
\end{document}
