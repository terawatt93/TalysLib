...
void TalysCalculation::ReadParametersFromFile(string filename)
{
	...
	{
		if(line.find("#")==string::npos)//если в строке файла нет символа # (иначе - это комментарий)
		{
			if(line.find("Target:")!=string::npos)//если в строке файла нашлось слово Target
			{
				stringstream s(line);
				s>>line>>Target;//считываем ядро - мишень
			}
...
{
	VarValues.resize(0);//удаляем в векторе все значения
	double value=min;
	if(step==0)
	{
...
{
	if(VarValues.size()>0)//если размер вектора с 
	{
		Nucleus Nucl(Target);
