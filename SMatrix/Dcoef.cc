...
map<tuple<int, int, int, int, double, double>, complexd> S1;
map<tuple<int, int, int, int, double, double>, double> Smod;
map<tuple<TString, int, int, double, double, TString>, double> Trans;
map<tuple<double, TString>, double> Tfull;
...
/*TF1 gamma()
{
	...
	{	auto j = i.first
		...
		if(Q == 0)
		{
			neutron+=TString::Format("%f*legendre(%d,cos(M_PI*x/180))+",i.second,lam);
		}
		else if(lam1 == 0)
		{
			gamma+=TString::Format("%f*legendre(%d,cos(M_PI*x/180))+",i.second,Q);
		}
	}
		neutron.Remove(kTrailing,"+")
		gamma.Remove(kTrailing,"+")
}
*/
...
class Dlam
{
	int lam = 0, lam1 = 0, Q = 0;
	TF1 phi;
	complexd result = complexd(0,0);
	double beg;
	int L = 2;
	int Ii = 2, If = 0, Ia = 0;
	double s = 0.5;
...
int Dlam::addS()
{
	...
	for (auto &a : lj()) 
	{
		for(auto &a1 : lj()) 
		{
			...
			if(coef==0) continue;
			complexd S = S1[make_tuple(2,1,la,lb,ja,jb)]*conj(S1[make_tuple(2,1,la1,lb1,ja1,jb1)]);
			//cout<<S<<endl;
			//cout<<coef<<endl;
			summa += S*coef;
			//cout<<summa<<endl;
...
int Dlam::addT()
{
	...
	for (auto &a : lj()) 
	{
		...
		double T = Trans[make_tuple("neutron",0,la,ja,J,parity)]
                       *Trans[make_tuple("neutron",1,lb,jb,J,parity)]/Tfull[make_tuple(J,parity)];
        //cout<<"trans "<<T<<endl;
		summa += T*coef;
		//cout<<summa<<endl;
	}
    ...
    for (int i = 0; i < nlinesInt; i++) {
      ...
      for (int j = 0; j < count; j++) {
       ...
       // string im;
        ...
        //cout<<line3<<endl;
        //cout<<"el "<<el<<" l_b "<<l_b<<" j_b "<<j_b<<endl;
        //cout<<im<<endl;
        reS.ReplaceAll("D","e");
        imS.ReplaceAll("D","e");
        SS.ReplaceAll("D","e");
  ...
  while (nl != int(lines.size())) {
   ...
   // cout << make_tuple(tip, level, l, j, J, parity) << endl;
	...
	for (auto &i : Trans) {
      ...
      //cout<<" J "<<J<<" par "<<parity<<endl;
      Tfull[make_tuple(J, parity)] += i.second;
      //cout << i.first << " " << make_tuple(J, parity) << " " << Tfull[make_tuple(J, parity)] << endl;
    }
  return 0;
...
void Dcoef()
{
	...
	//D.addS();
	//D.addT();
	//cout<<"result "<<D.pr_result()<<endl;

	map<tuple<int, int, int>, double> D;
	map<tuple<int, int, int>, string> d;
    ...
    // for (int Q = 0; Q < 1; Q++) {
		for (int lam1 = 0; lam1 < 7; lam1++) {
			for (int lam = abs(Q - lam1); lam <= Q + lam1; lam++) {
				Dlam dl(lam, lam1, Q);
            // d[(lam, lam1, Q)] = beq + "=";
				//string beg = beginning(lam, lam1, Q)[1];
				dl.addS();
				dl.addT();
				if (dl.pr_result() != 0) { 
	...
	//TF1 *f2 = new TF1("pol0","pol0",0,180);
